<template>
    <form @submit.prevent="handleSubmit" style="height: 10rem">
        <div class="mb-2">
            <div v-if="feedback.status.value == FeedbackStatus.none" class="text-info">{{ $t("auth.fillInUsernameMsg")
                }}</div>
        ...
        <LoadingContainer :is-loading="isLoading">
            <div class="row mb-2">
                ...
                <div class="col">
                    <input type="text" class="form-control" name="username" v-model="username" autocomplete="username"
                        required :readonly="isSuccess" />
                </div>
            </div>
            <div class="row">
                <div class="col">
                    ...
                    <button v-else type="submit" class="btn btn-primary" :disabled="!isFormValid">{{ $t("submit")
                        }}</button>
                </div>
                <div class="col-auto">
