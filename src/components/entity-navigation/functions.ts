...
export function useNavigation() {
    ...
    console.debug("useNavigation", { groups, dashboard, navbar, search, sidebar, configs })
    ...
    const dashboardTree = computed(() => {
        dashboardItems.value = importDashboard({ groups: groups!, entities: dashboard!, configs, hasAccess })
        return buildNavigationTree(dashboardItems.value)
    })
    const navbarItems = ref<Array<INavCore>>()
    const navbarTree = computed(() => {
        navbarItems.value = importNavbar({ groups, entities: navbar!, configs, hasAccess }) as Array<INavItem>
