...
const items = computed(() => facetGroup.value.parentFacets ?? [])
...
function handleAdd(item?: Facet) {
    facetGroup.value.parentFacets ??= []
    facetGroup.value.parentFacets.push(FacetGroupFacet.create({ facetGroupId: facetGroup.value.id, facetId: item?.id, facet: item }))
}
</script>
