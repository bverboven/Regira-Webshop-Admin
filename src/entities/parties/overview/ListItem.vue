    ...
    <div class="row border-bottom border-bottom-1 py-2">
        ...
        <div class="col d-none d-sm-block">
            <template v-if="item.$address != null">
                <AddressButton :modelValue="item.$address" class="btn btn-default p-1" /> {{ getLocation(item) }}
            </template>
        </div>
...
const getContactData = computed(() => (item: Entity) => distinctBy(item.contactData || [], (cd: ContactDetails) => cd.dataType).filter(x => [ContactDataTypes.email, ContactDataTypes.phone].includes(x.dataType!)) as Array<ContactDetails>)
