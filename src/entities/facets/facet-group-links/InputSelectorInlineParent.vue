...
const items = computed(() => facet.value.facetParentGroups ?? [])
...
function handleAdd(item?: FacetGroup) {
    facet.value.facetParentGroups ??= []
    facet.value.facetParentGroups.push(FacetFacetGroup.create({ facetId: facet.value.id, facetGroupId: item?.id, facetGroup: item }))
}
</script>
