...
fetch(`${appConfig.baseUrl}/config.json?v=${formatDateTime(new Date(), "yyyyMMdd")}`)
  ...
  .then(async (config: Record<string, any>) => {
    ...
    const translations = await fetch(
      `${appConfig.baseUrl}/data/translations.json?v=${formatDateTime(new Date(), "yyyyMMdd")}`,
    ).then((r) => r.json());
    const appTranslations = await fetch(
      `${appConfig.baseUrl}/data/app-translations.json?v=${formatDateTime(new Date(), "yyyyMMdd")}`,
    ).then((r) => r.json());
    ...
    const appIcons = await fetch(
      `${appConfig.baseUrl}/data/app-icons.json?v=${formatDateTime(new Date(), "yyyyMMdd")}`,
    ).then((r) => r.json());
